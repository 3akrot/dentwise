...
### TC-06: Doctor Active Status Filtering (EP - Active/Inactive Partitions)

| Field               | Value                                                                                   |
| ------------------- | --------------------------------------------------------------------------------------- |
| **Test ID**         | TC-06                                                                                   |
| **Requirement**     | FR-2 (Appointment Booking), FR-6 (Admin Doctor Management)                              |
| **Technique**       | Equivalence Partitioning                                                                |
| **Description**     | Test that only active doctors appear in the booking selection list                      |
| **Partitions**      | Active: `{doctors where isActive = true}`, Inactive: `{doctors where isActive = false}` |
| **Precondition**    | Database has 2 active doctors and 1 inactive doctor                                     |
| **Test Cases**      |                                                                                         |
| Case A (Active)     | Doctor with isActive=true → Should appear in booking selection list                     |
| Case B (Inactive)   | Doctor with isActive=false → Should NOT appear in booking selection list                |
| **Expected Output** | Only 2 doctors displayed in the selection (active ones only)                            |
| **Rationale**       | Tests that `getAvailableDoctors()` correctly filters by active status partition         |
...
| TC-06   | EP        | Doctor Active Status   | Active/Inactive partitions      |
